...
contract Deploy is Script {
    ...
    function grantRole(
        ...
        DelMEMO delmemo = DelMEMO(delMEMOProxy);
    ...
    function mintDelMemo(
        ...
        DelMEMO(delMEMOProxy).mint(address(settlementProxy), mintValue);

        uint256 balAfterMint = IERC20(memoToken).balanceOf(msg.sender);
        console.log("Deployer's balance: ", balAfterMint, " attomemo");

        bal = DelMEMO(delMEMOProxy).balanceOf(address(settlementProxy));
